...
__constant__ float L[9] = { 0, -1, 0, -1, 4, 1, 0, -1, 0 };

float grad = 0.0f;

__global__ void Laplacian::laplacianKernel(float* input,
    float* output,
    int width,
    int height) {
    ...
    for (int ky = -1; ky <= 1; ky++) {
        ...
        for (int kx = -1; kx <= 1; kx++) {
            ...
            grad += pixel * L[kernel_idx];
        }
    }

    const float magnitude = fminf(fabsf(grad), 255.0f);
    
    int idx = y * width + x;
    output[idx] = magnitude;
}

cudaError_t Laplacian::compute(float* input,
    float* output,
    int width,
    int height) {
    ...
    laplacianKernel << <grid,block >> > (dev_input, dev_output,
        width, height);
